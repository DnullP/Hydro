...
function getBaseSchema(def, allowUnsafe) {
    switch (def.type) {
        case 'string':
            return {
                type: 'string',
        ...
        case 'dict':
            if (!allowUnsafe)
                throw new Error('dict is unsafe, set allowUnsafe=true to ignore.');
            return {
                type: 'object',
...
function convertDef(def, allowUnsafe = false) {
...
function getAllRefs(def) {
    const refs = { ...def.refs };
    if (def.list?.length) {
        for (let i = 0; i < def.list.length; i++) {
            if (typeof def.list[i] === 'number')
                continue;
            Object.assign(refs, getAllRefs(def.list[i]));
            def.list[i] = def.list[i].uid;
        }
    }
    if (def.dict) {
...
function convert(schema, allowUnsafe = false) {
    ...
    for (const key in refs)
        defs[key] = convertDef(refs[key], allowUnsafe);
